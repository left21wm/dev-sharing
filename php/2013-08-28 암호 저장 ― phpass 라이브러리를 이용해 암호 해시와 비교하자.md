...
- phpass 0.3에서 테스트했다고 한다.
- 공식 웹사이트에 들어가 보니, pH pass로 발음하라고 한다. ㅋ

해싱은 사용자 암호를 DB에 저장하기 전에 보호하는 표준 방법이다. MD5나 SH1같은 많은 일반적 해싱 알고리즘들은 암호 저장을 하는데 안전하지 않다. 왜냐하면 해커들이 이런 일반적인 해싱 알고리즘들을 이용해 쉽게 해싱된 암호를 깨기 때문이다.

패스워드를 해싱하는 가장 안전한 방법은 bcrypt 알고리즘[1]을 이용하는 것이다. 오픈 소스 phpass 라이브러리는 클래스를 사용하기 쉬운 기능을 제공한다.
